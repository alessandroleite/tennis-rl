...
class DDPGAgent:
    ...
    def __init__(self, id, state_size, action_size, seed, memory, num_agents, device="cpu", batch_size=200, lr_actor=1e-4, lr_critic=1e-3, gamma=0.99, weight_decay=0, tau=1e-3, update_frequency=20, n_learns=1):
        ...
        ======
            ...
            device (string): computing device (i.e. CPU, GPU) where the operations must be executed
        ...
        # Noise process
        self.noise = Ornstein(action_size, seed)
    ...
    def learn(self, experiences, gamma):
        ...
        all_next_actions = torch.cat(local_all_next_actions, dim=1).to(self.device)
        ...
        actions_pred = torch.cat((actions_pred, other_actions_pred), dim=1).to(self.device)
